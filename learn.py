...
@app.get('/book')
def get_book(q: str = Query(None, minlength=2, max_length=5)):#для Query нужен параметр.  Query это тот текст в ссылке который будет идти после знака ?. Указали мин и макс длину строки q 
    return q



#запуск приложения. Теперь можно запускать файл питона чтобы запустился сервер
